...
- The layout under `~/.katapult/template/` mirrors the **full** katapult template (the same root as the built-in `project_template/`). Most overrides live under a directory named **`__project_slug__`**, which is renamed to **`{{cookiecutter.project_slug}}`** in the merged copy—matching the stock template’s project directory. You can also place files at the template root (for example next to `cookiecutter.json` or under `hooks/`) if you need to override those paths.
